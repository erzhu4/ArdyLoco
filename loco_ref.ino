...
//config
const int maxTranslatedInput = 180;
const int stopTranslatedInput = 64;
const int acceleration = 200;
const int decceleration = 200;
      ...
      if (client.available()) {             // if there's bytes to read from the client,
        ...
        if (c == '\n') {                    // if the byte is a newline character
          ...
          if (currentLine.length() == 0) {
            ...
            client.print("<button class='blue' type='submit' onmousedown='location.href=\"/6\"'>STOP</button>");
